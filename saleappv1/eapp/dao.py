...
def add_user(name, username, password, avatar):
    if len( username) < 5:
        raise ValueError ('Username phai co toi thieu 5 ky tu')
    if len(password) <8:
        raise ValueError ('Password phai co toi thieu 8 ky tu')
    if not re.search(r'[0-9]',password):
        raise ValueError ('Password phai co ky tu so')
    if not re.search(r'[a-zA-Z]',password):
        raise ValueError ('Password phai co ky tu thuong')
